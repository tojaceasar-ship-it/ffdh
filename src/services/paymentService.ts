...
// Payment service API keys from environment (server-side only)
...
export const paymentService = {
  ...
  async initiateStripePayment(paymentData: any) {
    try {
      ...
      params.append('amount', Math.round(paymentData.amount * 100).toString()); // Convert to cents
  ...
  async initiatePayPalPayment(paymentData: any) {
    try {
      // Validate required fields
      if (!paymentData.amount || paymentData.amount <= 0) {
        throw new Error('Invalid payment amount');
  ...
  async initiatePayment(paymentData: any, provider: string = 'stripe') {
    if (provider === 'stripe') {
  ...
  async createPrintfulOrder(orderData: any) {
    try {
      // Validate order data
